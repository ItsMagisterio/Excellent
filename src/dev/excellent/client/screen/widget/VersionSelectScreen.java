...
public class VersionSelectScreen implements IAccess, IScreen, IMouse {
    ...
    private final Animation expandAnimation = new Animation(Easing.LINEAR, 150);
    private final Font bold = Fonts.INTER_BOLD.get(15);
    private final Font regular = Fonts.INTER_MEDIUM.get(15);
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (mc.currentScreen instanceof MultiplayerScreen) {
            ...
            if (isExpand && (expandAnimation.isFinished() || !isExpand)) {
                RectUtil.drawRect(matrixStack, x + width - 2 - 0.25F, scrollY, x + width - 2 - 0.25F + 1, scrollY + height - (margin + 2) - padding / 2F, ColorUtil.getColor(40, 40, 40, alpha));
                scroll.render(matrixStack, new Vector2f(x + width - 2, scrollY), height - (margin + 2) - padding / 2F, alpha);
            }
            ...
            for (ProtocolVersion protocol : Excellent.getInst().getViaMCP().getPROTOCOLS()) {
                ...
                RectUtil.drawRoundedRectShadowed(matrixStack, x + 2, positionY - (regular.getHeight() / 2F) + 2, x + width - 6, positionY - (regular.getHeight() / 2F) + 2 + (regular.getHeight() * 2) - 4, 1, selected ? 2 : 1, protocolColor, protocolColor, protocolColor, protocolColor, true, true, true, true);
            ...
            scroll.setMax(height - totalHeight - (padding * 2) - (padding / 2F));
    ...
    private void selectVersion(double mouseX, double mouseY, int button) {
        ...
        if (isHover(mouseX, mouseY, x, y, width, margin) && (isLClick(button) || isRClick(button))) {
            isExpand = !isExpand;
        } else if (isHover(mouseX, mouseY, x, y + margin + 2.5, width, height - margin - 2.5)) {
            float positionY = (float) (y + (margin + 8 + scroll.getScroll()));

            for (ProtocolVersion protocol : Excellent.getInst().getViaMCP().getPROTOCOLS()) {
                if (isHover(mouseX, mouseY, x + 5, positionY - regular.getHeight() / 2F, width - 10, regular.getHeight() * 2) && isLClick(button)) {
